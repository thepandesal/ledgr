...
export default function PayModal({ visible, onClose, recording, items, filledPeople, step, setStep, mode, setMode, manualAmount, setManualAmount, selectedPeople, setSelectedPeople, accounts, account, setAccount, date, setDate, complete, setComplete, loading, getAmount, onConfirm, spaces, spaceId, setSpaceId, defaultSpaceId }: Props) {
  ...
  return (
    <BottomSheet visible={visible} onClose={onClose} sub="payable" title={step === 'account' ? 'paying from' : 'pay bill'}>
      {step === 'account' ? (
        <>
        ...
        <View style={{ gap: 12, width: '100%' }}>
          ...
          {/* Space picker */}
          <View style={{ gap: 8 }}>
            <Text style={formStyles.hintMuted}>record payment in</Text>
