...
export default function RecordingDetailScreen() {
  ...
  const confirmCollectDue = async () => {
    ...
    try {
      ...
      loadPaymentData();
      // Sync tagged friends' mirrored expense
      syncTaggedExpenses(amount, cappedPaid, newStatus);
      if (excess > 0.01) {
  ...
  const confirmCollect = async () => {
    ...
    try {
      ...
      loadPaymentData();
      syncTaggedExpenses(amount, cappedPaid, collectComplete ? 'paid' : 'partial');
      if (excess > 0.01) {
        setOverpaymentAmount(Math.round(excess * 100) / 100);
        setOverpaymentModal(true);
      }
  ...
  const syncTaggedExpenses = async (paymentAmount: number, totalPaid: number, status: string) => {
    ...
    try {
      ...
      for (const tag of tags) {
        if (!tag.tagged_user_id) continue;
        // Upsert the mirrored expense for User B
        if (tag.mirrored_expense_id) {
          ...
          }).eq('id', tag.mirrored_expense_id);
        } else {
          // Create new expense for User B
          const { data: newExp } = await supabase.from('recordings').insert({
            ...
            notes: `from ${userName || 'someone'} · auto-recorded`,
            is_tagged: true,
          }).select('id').single();
          if (newExp?.id) {
            await supabase.from('recording_tags').update({ mirrored_expense_id: newExp.id }).eq('id', tag.id);
          }
        }
        // Notify User B
        await supabase.from('notifications').insert({
          ...
          title: `${userName || 'Someone'} logged a payment`,
          body: `${recording?.name ?? 'expense'} · ${totalPaid.toLocaleString('en-US', { minimumFractionDigits: 2 })} paid so far`,
  ...
  const openTagFriendModal = async () => {
    ...
    setTagFriends(names);
    // Load existing tags for this recording
    const { data: tags } = await supabase
      .from('recording_tags')
      .select('*, tagged_user:tagged_user_id(id)')
      .eq('recording_id', recordingId)
  ...
  const confirmTagFriend = async () => {
    ...
    try {
      ...
      const alreadyTagged = existingTags.some(t => t.tagged_user_id === tagSelectedFriend.id);
      ...
      await supabase.from('recording_tags').insert({
        ...
        tagged_user_id: tagSelectedFriend.id,
        amount,
        status: 'pending',
      });
      // Send notification
      await supabase.from('notifications').insert({
        ...
        body: `Tap Requests in Notifications to view and respond.`,
        data: { recordingId, taggerName: userName },
        status: 'new',
        is_read: false,
      });
      ...
      const { data: tags } = await supabase.from('recording_tags').select('*').eq('recording_id', recordingId).in('status', ['pending', 'accepted']);
      setExistingTags(tags ?? []);
    } catch (e: any) {
      setTagError(e.message ?? 'something went wrong');
    } finally {
      setTagLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (!recordingId || !userId) return;
    supabase.from('recording_tags').select('*, tagged_user_id').eq('recording_id', recordingId).in('status', ['pending', 'accepted'])
  ...
  return (
    <Animated.View style={[{ flex: 1, backgroundColor: Colors.white }, { transform: [{ translateX: slideAnim }] }]}>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={rd.scroll} showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ paddingVertical: 12 }} contentContainerStyle={{ paddingHorizontal: PAGE, gap: 8 }}>
            {recording?.type === 'expense' && !linkedPayable && !linkedReceivable && !recording?.is_due && !recording?.is_tagged && (
            ...
            {recording?.type === 'due' && recording?.status !== 'paid' && !recording?.is_tagged && (
            ...
            {recording?.type === 'due' && recording?.status !== 'paid' && recording?.is_tagged && (
              <View style={[rd.actionChip, { backgroundColor: Colors.surface }]}>
                <Ionicons name="lock-closed-outline" size={13} color={Colors.muted} />
                <Text style={[rd.actionChipText, { color: Colors.muted }]}>managed by tagger</Text>
              </View>
            )}
          ...
          <View style={{ paddingHorizontal: PAGE }}>
            ...
            {recording?.is_tagged && (
              <View style={rd.infoRow}>
                <Text style={rd.infoLabel}>tagged by</Text>
                <Text style={[rd.infoValue, { color: ACCENT_DARK }]}>payment managed by tagger</Text>
              </View>
            )}
            {linkedPayable && (
      ...
      <BottomSheet visible={tagFriendModal} onClose={() => { setTagFriendModal(false); setTagError(''); }} title="tag a friend">
        ...
        <Text style={{ fontFamily: Brand.font.monoBold, fontSize: 10, color: Colors.muted, letterSpacing: 0.6, textTransform: 'uppercase', marginBottom: 8 }}>amount they owe</Text>
        <TextInput
        ...
        {tagFriends.length === 0 ? (
          ...
          <ScrollView style={{ maxHeight: 200 }} showsVerticalScrollIndicator={false}>
            {tagFriends.map(f => {
              const alreadyTagged = existingTags.some(t => t.tagged_user_id === f.id);
              return (
                <TouchableOpacity
                  key={f.id}
                  style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: Colors.border, gap: 10, opacity: alreadyTagged ? 0.4 : 1 }}
                  onPress={() => !alreadyTagged && setTagSelectedFriend(f)}
        ...
        <TouchableOpacity
          style={[rd.doneBtn, { marginTop: 16, opacity: (!tagSelectedFriend || !tagAmount || tagLoading) ? 0.4 : 1 }]}
          onPress={confirmTagFriend}
          disabled={!tagSelectedFriend || !tagAmount || tagLoading}
        >
