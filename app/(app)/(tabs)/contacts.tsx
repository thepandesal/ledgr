...
export default function ContactsScreen() {
  ...
  const { data: outgoing = [] } = useQuery<{ id: string; receiver_name: string; receiver_code: string }[]>({
    ...
    queryFn: async () => {
      ...
      const ids = data.map((r: any) => r.receiver_id);
      const { data: settings } = await supabase
        .from('user_settings').select('user_id, profile_code').in('user_id', ids);
      const names = await Promise.all(
        ids.map((id: string) =>
    ...
    queryFn: async () => {
      ...
      const names = await Promise.all(
        friendIds.map((id: string) =>
          supabase.rpc('get_user_display_name', { user_id: id }).then(({ data }) => ({ id, name: data ?? 'unknown user' }))
        )
      );
    ...
    queryFn: async () => {
      ...
      return data.map((r: any) => {
        ...
        return {
          ...
          requester_name: nameRow?.name ?? 'unknown user',
          requester_code: setting?.profile_code ?? '',
        };
      });
    },
    enabled: !!userId,
    ...
    <SafeAreaView style={s.container}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
            ...
            <View style={s.list}>
              {incoming.map(req => (
                <View key={req.id} style={s.row}>
                  <View style={s.avatar}>
                    <Text style={s.avatarText}>{(req.requester_name ?? '?').charAt(0).toUpperCase()}</Text>
                  </View>
                  ...
                  >
                    {responding === req.id
                      ...
                      : <Ionicons name="checkmark" size={14} color={ACCENT_DARK} />}
                  </TouchableOpacity>
                </View>
              ))}
            </View>
        ...
        {isLoading ? (
          ...
          <View style={s.emptyWrap}>
            ...
            <Text style={Brand.type.emptyText}>
              {contacts.length === 0
                ? 'contacts added via split bill appear here'
          ...
          <View style={s.list}>
            {filtered.map(c => (
              <View key={c.id} style={s.row}>
                ...
                <TouchableOpacity
                  onPress={() => { setSelected(c); setDeleteModal(true); }}
      ...
      <BottomSheet visible={deleteModal} onClose={() => setDeleteModal(false)} title={selected?.name?.toLowerCase() ?? 'contact'}>
        ...
        <TouchableOpacity style={s.deleteBtn} onPress={handleDelete} activeOpacity={0.8}>
          <Text style={s.deleteBtnText}>remove contact</Text>
        </TouchableOpacity>
        <TouchableOpacity style={s.cancelBtn} onPress={() => setDeleteModal(false)} activeOpacity={0.8}>
      ...
      <BottomSheet visible={addModal} onClose={closeAddModal} title="add friend">
        ...
        <View style={s.codeInputRow}>
          <TextInput
            style={s.codeInput}
            placeholder="LDGR-XXXX"
            placeholderTextColor={Colors.faint}
            value={codeInput}
            onChangeText={v => { setCodeInput(v.toUpperCase()); setAddError(''); setAddResult(null); setAddSuccess(false); }}
          ...
          />
          <TouchableOpacity
            style={[s.searchBtn, (!codeInput.trim() || addSearching) && { opacity: 0.4 }]}
            onPress={searchByCode}
            disabled={!codeInput.trim() || addSearching}
            activeOpacity={0.8}
          >
            {addSearching
              ? <ActivityIndicator size="small" color={ACCENT_DARK} />
              : <Ionicons name="search-outline" size={16} color={ACCENT_DARK} />}
          </TouchableOpacity>
        </View>
        ...
        {addResult && !addSuccess ? (
          <View style={s.addResultWrap}>
            <View style={s.avatar}>
              <Text style={s.avatarText}>{addResult.name.charAt(0).toUpperCase()}</Text>
            </View>
            <View style={{ flex: 1, gap: 2 }}>
            ...
            <TouchableOpacity
              style={[s.acceptBtn, { paddingHorizontal: 14, width: 'auto' as any, borderRadius: Radius.pill }]}
              onPress={sendRequest}
              disabled={addSending}
              activeOpacity={0.8}
            >
              {addSending
                ? <ActivityIndicator size="small" color={ACCENT_DARK} />
                : <Text style={{ fontFamily: Brand.font.monoBold, fontSize: 12, color: ACCENT_DARK }}>send request</Text>}
            </TouchableOpacity>
          </View>
        ) : null}
      </BottomSheet>
...
const s = StyleSheet.create({
  ...
  searchWrap: {
    ...
    borderWidth: 1, borderColor: Colors.borderMid, borderRadius: Radius.pill,
