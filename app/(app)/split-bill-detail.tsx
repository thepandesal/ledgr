  ...
  const saveAsImage = async () => {
    ...
    try {
      ...
      const rows = filledPeople.map((p: string) =>
        `<tr><td style="font-family:monospace;padding:10px 0;border-bottom:1px dotted #ddd;font-size:14px">${p}</td>`+
        `<td style="font-family:monospace;font-weight:bold;text-align:right;padding:10px 0;border-bottom:1px dotted #ddd;font-size:14px;color:${totals[p]<0?'#7fd8cd':'#425252'}">`+
        `${totals[p]<0?'-':''}${Math.abs(totals[p]).toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr>`
      ).join('');
      const itemRows = items.map((item: any) => {
        const d = isDeductType(item.recording_type);
        return `<div style="background:#fafafa;border-radius:10px;padding:14px;margin-bottom:10px;border:1px solid #eee">`+
          `<div style="display:flex;justify-content:space-between;margin-bottom:6px">`+
          `<span style="font-family:monospace;font-weight:bold;font-size:14px">${item.name}</span>`+
          `<span style="font-family:monospace;font-size:13px;color:${d?'#7fd8cd':'#ff7043'}">${d?'-':'+'}${Number(item.cost).toLocaleString('en-US',{minimumFractionDigits:2})}</span></div>`+
          (item.people?.length>0?`<div style="font-family:monospace;font-size:11px;color:#999">${item.people.join(', ')}</div>`:'')+`</div>`;
      }).join('');
      const accHtml = shareAccounts.filter((a: any) => shareSelectedIds.includes(a.id)).map((a: any) =>
        `<div style="background:#d8efea;border-radius:10px;padding:14px;margin-bottom:10px">`+
        `<div style="font-size:15px;font-weight:600">${a.account_name}</div>`+
        `<div style="font-family:monospace;font-size:11px;color:#666">${a.bank}</div>`+
        `<div style="font-family:monospace;font-weight:bold;font-size:14px">${a.account_number}</div></div>`
      ).join('');
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>`+
        `body{margin:0;padding:32px;background:#fff;font-family:sans-serif;width:480px}*{box-sizing:border-box}</style></head><body>`+
        `<div style="font-size:20px;color:#7fd8cd;font-weight:900;margin-bottom:20px">LEDGR</div>`+
        `<div style="font-size:24px;font-weight:600;color:#425252;margin-bottom:4px">${name}</div>`+
        `<div style="font-family:monospace;font-size:11px;color:#999;margin-bottom:28px">${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div>`+
        `<div style="font-size:13px;color:#7fd8cd;font-weight:700;margin-bottom:10px;text-transform:uppercase;letter-spacing:1px">Per Person</div>`+
        `<div style="background:#fafafa;border-radius:10px;padding:8px 16px;margin-bottom:24px;border:1px solid #eee"><table style="width:100%;border-collapse:collapse">${rows}</table></div>`+
        `<div style="font-size:13px;color:#7fd8cd;font-weight:700;margin-bottom:10px;text-transform:uppercase;letter-spacing:1px">Items</div>${itemRows}`+
        (accHtml?`<div style="font-size:13px;color:#7fd8cd;font-weight:700;margin:20px 0 10px;text-transform:uppercase;letter-spacing:1px">Payment</div>${accHtml}`:'')+
        `<div style="font-family:monospace;font-size:10px;color:#ccc;text-align:center;margin-top:28px">generated by LEDGR</div>`+
        `</body></html>`;

      if (Platform.OS !== 'web') {
        const Print = require('expo-print');
        const Sharing = require('expo-sharing');
        const { uri } = await Print.printToFileAsync({ html, width: 480 });
        ...
        // Render HTML in hidden iframe then capture
        const iframe = document.createElement('iframe');
        iframe.style.cssText = 'position:fixed;left:-9999px;top:0;width:480px;height:800px;border:none;background:#fff';
        ...
        canvas.toBlob((blob: Blob | null) => {
          ...
          a.href = url;
          a.download = `${String(name).replace(/\s+/g, '-')}-split.png`;
